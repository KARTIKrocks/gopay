 ...
 *
 *   - versions are MAJOR.MINOR only ("0.9"), never patch ("0.9.0")
 ...
 * deprecation. Purely additive releases get a `_0.9+_` marker in docs/ instead,
 * which is why the versions here are expected to be sparse. See VERSIONING.md
 * rules 1 and 2. This script cannot check that condition, so it is on you.
 *
 * Usage: npm run cut-version -- 0.9
 */
...
if (!/^\d+\.\d+$/.test(version)) {
  fail(
    `"${version}" is not a MAJOR.MINOR version.\n\n` +
      '  Snapshots are cut per minor release only — "0.9", not "0.9.0" or "v0.9".\n' +
