...
func TestAmountValidateAcceptsLowercase(t *testing.T) {
	// Validate canonicalizes the currency in place (trim + uppercase) so that
	// downstream providers always receive a clean ISO 4217 code.
	a := &Amount{Value: 100, Currency: "  usd  "}
	if err := a.Validate(); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if a.Currency != "USD" {
		t.Errorf("Currency = %q, want USD (canonicalized by Validate)", a.Currency)
	}
